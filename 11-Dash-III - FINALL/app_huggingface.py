...
def translate(n_clicks, src_lang, tgt_lang, src_text):
    ...
    translated = model.generate(**tokenizer.prepare_seq2seq_batch(text_in, return_tensors="pt"))
    tgt_text = [tokenizer.decode(t, skip_special_tokens=True) for t in translated]
    print("text_in = ",tgt_text)
    #batch = tokenizer.prepare_seq2seq_batch(text_in)
    #print("batch = ", batch)
    
    #model_inputs = tokenizer(text_in)
    #print(f'check = {model_inputs}')
    #for k in batch:
